...
<html lang="en">
    ...
    <body >
     ...
     <script src="https://code.jquery.com/jquery-3.1.1.min.js"></script>
       ...
       <script>
        $.ajax({   
                            url:"/rrr-paymentstatus-pin.php?user=<?php echo $username; ?>&rrr=<?php echo $rrrget;?>&orderId=<?php echo $orderIDget; ?>",
                            type: 'POST',
                            cache:false,
                            //data: dataString,
                            beforeSend: function() {},
                            ...
                            success: function(response) {
                                var data = JSON.parse(response);
                                 $("#response").html(data.msg+', Please wait...');
                               // alert("Deposit Placed");
                                ...
                                buttonsStyling: false,
                                 ...
                                 if(data.msg == 'Payment successful and approved'){
                                    setTimeout(function() { 
                                    var msd = 'Please enter the pin sent to your email / Pin is also out filled in the box below. Note: pin is valid only three times'
                                     window.location.href = "/resultcheck.php?msg="+msd+'&pin='+data.pin;
                                    }, 10000);
                                }
       ...
       <script>
        $.ajax({   
                   url:"/rrr-paymentstatus-pin.php?user=<?php echo $username; ?>&rrr=<?php echo $rrrget; ?>&statusGet=<?php echo $statusget; ?>&statusCodeGet=<?php echo $statuscodeget; ?>",
                            type: 'POST',
                            cache:false,
                            //data: dataString,
                            beforeSend: function() {},
