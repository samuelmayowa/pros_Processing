<?php //$dd = $y=date("Y"). '/' .($d=date("Y")); ($d+=1); 
$ddd= $y=date("Y") .'/'. $d=date("Y")+1;  
?>
...
$applType=='';
//=========== Get User Accounts Details and Tye Of Application
if(!isset($_SESSION['user'])){
    ...
    if(!$query){
        ...
        while($results = mysqli_fetch_array($query)){
            ...
            $email = $results['Email'];
            $applType=$results['ApplicationType'];
            ...
            $_SESSION['user_id'] = $id;
                                            ...
                                            // Fetch Amount based on Type Of Aplication
                                                $amount="";
                                                $y='';
                                                //echo $dd=$y.=date("Y").  $y .='/' . $d=date("Y") .$d+=1;
                                                $ddd.=' POST UTME Application Form';
                                                echo "<script>alert('$ddd'); </script>";
                                                //if($applType === $ddd){
                                                    if($applType == '2021/2022 POST UTME Application Form'){
                                                    $sql_department = "SELECT Amount FROM paymentCategories WHere payCategories='Post UTME Application Fees'";
                                                $fees = mysqli_query($con,$sql_department);
                                                while($row = mysqli_fetch_assoc($fees)){
                                                   
                                                    echo $amount1 = $row['Amount'];
                                                      }
                                                }elseif($applType == $y=date("Y") .'/'. $d=date("Y")+1 . ' Direct Application'){
 ...
 $type_notPaid = '2020/2021 Application Form';
 $type_Paid = '2020/2021 Application Form';
    $status_notPaid = 'Payment Reference generated';
    $status_Paid = 'Successful';
    $stdID = $user_id = $_SESSION['user_id'];
    $rrdata_notPaid = getinvoiceWithType($stdID,$status_notPaid,$type_notPaid,$con);
    $rrdata_Paid = getinvoiceWithType($stdID,$status_Paid,$type_Paid,$con);
    ...
    $amount = $rrdata_notPaid['Amount'];
    $refNNum= $rrdata_notPaid['RefNum'];
    $status=$rrdata_notPaid['InvoiceStatus'];
   // $statusCode = $rrdata_notPaid['rrr_statuscode'];
    $orderID= $rrdata_notPaid['RefOrderID'];
    $stdID= $_SESSION['userID'];
    /*if(empty($rrdata_notPaid)){
    header("location:UserdashBoard.php?user={$username}");
}*/if(!empty($rrdata_notPaid)){
    header("location:generateInvoice.php");
}elseif(!empty($rrdata_Paid)){
    header("location:print_success_invoice.php");
}
                                    ...
                                    <div class="card-body">
                                        <form action="generateInvoice.php" method="POST"> 
                                            ...
                                            <div class="form-row">
                                               <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control py-4" id="inputPayingFor" name="payingFor" type="text" placeholder="What Are You Paying For?" value="<?php if(isset($applType)) { echo $applType;  } else {  $y=date("Y"); echo $y .'/';  $d=date("Y"); echo $d+=1; echo  'Application Form'; } ?>"  readonly/>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <input class="form-control py-4" id="inputamount" name="amount" type="tex" aria-describedby="amountHelp" placeholder="Enter Amount"   value="<?php if(isset($amount)){ echo $amount; }else { echo $amount1; } ?>" readonly/>
                                            </div>
                                            </div>
                                            </div>
...
function clickButton() {
  ...
  s.src = "https://applications.escohsti-edu.ng/rrr-generate-application.php?user=<?php echo $user;?>";
