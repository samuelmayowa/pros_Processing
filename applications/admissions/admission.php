...
<?php 
   ...
   header('location:/UserdashBoard.php?user='.$user .' && adm=You have Not Paid For The  Admission That Has Been Provissioned for You Yet');
        ...
        $query = mysqli_query($con, $query) or die (mysqli_error($con));
            ...
            $usr=$_SESSION['user'];
            //$query="UPDATE Invoices SET InvoiceStatus='Successful' WHERE StudentID IN (SELECT ID From userAccounts WHERE username = (SELECT username from onlineApplications WHERE Acceptance=1))";
                   ?> 
