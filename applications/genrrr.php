...
if(!isset($_SESSION['user'])){
    ...
    $query_card = "SELECT * FROM scratchCards WHERE userId = '$stdID' AND pinCounts = '0'";
    $q_card = mysqli_query($con,$query_card);
    while($v = mysqli_fetch_assoc($q_card)){
        $status = $v['pinStatus'];
     }
     $query_card1 = "SELECT * FROM scratchCards WHERE userId = '$stdID' AND pinCounts < '3'";
    $q_card1 = mysqli_query($con,$query_card1);
    while($v1 = mysqli_fetch_assoc($q_card1)){
        $status1 = $v1['pinStatus'];
     }
     if($status == 'Successful' || $status1 == 'Successful'){
          header('location:resultcheck.php?msg=Enter pin sent to your email to check result!');
     }elseif($status == 'Payment Reference generated'){
         header('location:pinInvoice.php?msg=Pay to get pin!');
     }
}
                        ...
                        <div class="row justify-content-center">
                            <div class="col-lg-12">
                                <div class="card shadow-lg border-0 rounded-lg mt-5">
                                    ...
                                    <div class="card-header"><h3 class="text-center font-weight-light my-4">Generate Invoice Scratch Pin</h3></div>
                                    <div class="alert"></div>
...
<p id="status"></p>
                                    <div class="card-body">
                                        <form action="pinInvoice.php" method="POST"> 
                                            <div class="form-row">
                                                <div class="col-md-6">
                                            ...
                                            <div class="form-row">
                                               <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control py-4" id="inputPayingFor" name="payingFor" type="text" placeholder="What Are You Paying For?" value="2021/2022 Result scratch Pin"  readonly/>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="form-group mt-4 mb-0"><button type="submit" name="invoice" class="btn btn-primary btn-block">Generate payment</button></div>
                                        </form>
                                    </div>
...
function clickButton() {
  ...
  s.src = "rrr-generate-pin.php?user=<?php echo $user;?>";
